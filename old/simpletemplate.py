...
class SimpleTemplate(object):
    """
    template engine has simple function
    """
    ...
    def process(self, exit_pats=(), start_line=0, kws={}):
        ...
        while len(self.lines) > cur_line:
            ...
            for exit_pats in exit_pats:
                if exit_pats.search(line):
            ...
            if not pattern_found:
                    ...
                    cur_line+=1
        if exit_pats:
            raise Exception("End of line while parsing")
    ...
    def handle_value(self, _match, _line_no, _kws={}):
        ...
        while True:
            ...
            if not _m:
                break
            pos += _m.end()
            _rep.append((_m.group(1), eval(_m.group(1))))  # (3)
        for t, r in _rep:
    ...
    def handle_if(self, _match, _line_no, _kws={}):
        ...
        if not _cond:
            raise "SyntaxError: invalid syntax in line %d" % _line_no  # (1)
        _cond = _cond[:-1]
        locals().update(_kws)
